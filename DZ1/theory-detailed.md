...
- **Версии**: LTS-релизы — 8, 11, 17, 21, 25. Этот трек закреплён на **17**: все недельные проекты объявляют Gradle toolchain `JavaLanguageVersion.of(17)`, поэтому ставьте 17 (более новый JDK тоже сработает — toolchain сам скачает 17, но для единообразия материалов используется 17).
- **Дистрибутивы**: Eclipse Temurin (самый нейтральный), Amazon Corretto, Azul Zulu, GraalVM (даёт native-image), Oracle JDK.
- **Управление версиями**: SDKMAN! (`sdk install java 17-tem`), jenv, или встроенный в IDEA загрузчик JDK.

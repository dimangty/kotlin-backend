...
## Доработка 26 июля 2026: единая структура учебных материалов

- **README недель 6 и 3-1...6-1 приведены к стандартной структуре трека**: результат недели, таблица «теория - где она в коде», запуск, задания, что разобрать с ментором, критерий готовности, контрольные вопросы и материалы. Раньше эти пять README были на 18-35 строк и не отвечали структуре, обещанной в корневом README; каждая ссылка на файл и утверждение о поведении кода сверены с исходниками (`session-a/b.sql`, `FintechService.kt`, `IndexLabService.kt`, `PaymentHistoryService.kt`, `AccountService.kt`, `SerializableDebitService.kt`, миграции Flyway).
- **Теория связана с точками входа.** `THEORY-SHORT.md` и `THEORY-DETAILED.md` существовали во всех 20 каталогах, но не упоминались ни в одном README. Теперь каждый недельный README ссылается на оба файла сразу после результата недели, а корневой README перечисляет их в структуре проекта.
- **DZ1 согласован с треком**: рекомендация JDK 21 заменена на JDK 17, на котором закреплены Gradle toolchain всех недельных проектов; DZ1 добавлен в схему репозитория корневого README.

...
internal class ScenarioLog(private val title: String) {
    ...
    @Synchronized
    fun step(message: String) = add("•", message)

    /** Важный наблюдаемый итог нескольких SQL-операций. */
    @Synchronized
    fun result(message: String) = add("✓", message)

    /** Ожидаемая учебная ошибка, которая подтверждает механизм. */
    @Synchronized
    fun expected(message: String) = add("⚠", message)
    ...
    fun failure(error: Throwable) {
        ...
        add("✗", "${error.message ?: error::class.simpleName}$suffix")
    }
